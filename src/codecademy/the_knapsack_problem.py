...
but you're just one person who can only carry so much. Each item has a weight and value, and
your goal is to maximize the total value of items while remaining within the weight limit of
...
For example, let's say your knapsack can carry 10 units of weight. The item weights are [3, 6, 8]
and their values are [50, 60, 100]. Your knapsack function should return 110 since you can carry
the first and second items, whose values total 110. If you tried to carry the third item, which
        ...
        for j in range(1, len(subsolutions[0])):
            if weights[i - 1] <= j:
                subsolutions[i][j] = max(
                    values[i - 1] + subsolutions[i - 1][j - weights[i - 1]], subsolutions[i - 1][j]
                )
            else:
